...
export class CalcAverageRoomWallLengthHandlerAdapter implements ICommandHandler<number> {
    constructor(private commandDispatcher = new CommandDispatcher()) {
        this.commandDispatcher.register(CalcAverageAreaSidesLength.name, new CalcAverageAreaSidesLengthHandler());
    }

    handle(electrictDomainCommand: CalcAverageRoomWallLength): Observable<number> {
        const area = electrictDomainCommand.rooms.map((room: Room) => room.points.map((point) => new Point(point.x, point.y, 0)));
        const statisticsDomainCommand = new CalcAverageAreaSidesLength(area);
        return this.commandDispatcher.execute(statisticsDomainCommand).pipe(map((statistics: Statistics) => statistics.avgAreaSidesLength));
    }
}
